...
同 repo 的其他技能顶部写的都是某个具体用户名下的绝对路径,而那个用户名后来变了 ——
换机器或改 Windows 账户之后,那批路径**全部静默失效**:不报错,只是引用的文件读不到。

本技能一律用 `%USERPROFILE%` / `~`。这条在公开仓库里还有第二层意义:
写死的绝对路径会把本机用户名一并写进 git 历史,而且删不掉。
