...
### 主任务：鲜艳 Pop 风格第二版
客户给了第 4 张参考图（Solar Pop Studio，橘粉撞色创意工作室网站），要**参考它的设计元素**：

- **不规则 blob 形状组合** — 有机形状色块当背景
- **星芒 / 四瓣花 / 涂鸦线条**等装饰元素
- **鲜艳配色** — 高饱和橘红 + 桃粉 + 奶油底，纯暖色系无冷色
- **粗体全大写标题 + 手写体点缀字**（双字体搭配）
- **整页圆角容器 + 分区色块**，每区一个圆角面板不同色调
- 圆形旋转文字徽章、小圆形图标 chip、数字统计色块

**要求**：
- 新建 `demos/serai-beauty-jb-pop.html`，**不要动 `serai-beauty-jb.html`**
- 两版并存给客户对比，选定后再删另一版
- 客群仍是 25-45（注意：高饱和橘粉容易读起来偏年轻/廉价，要靠更多留白、克制手写体使用量来撑住 RM 288 疗程的价位感）
- 内容、区块顺序、轮播行为照搬现有版本，只换视觉

**技术提示**：blob / 星芒这些形状用 **CSS + 内联 SVG** 做，不要去找图片。可缩放、支持深浅色、零额外载入。设计 skill 平常不鼓励手绘 SVG 装饰，但这里是客户明确指定要这些形状，属于 explicit-brief 例外。
