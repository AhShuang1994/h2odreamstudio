...
> ⚠️ **2026-09-15 更新：暖陶土版 `demos/serai-beauty-jb.html` 已删除，不再保留。**
> 它整页的图都是 `picsum.photos` 随机占位图（hero 是一张手机特写），不能当作品展示。
> **留下的是 Pop 版 `demos/serai-beauty-jb-pop.html`** —— 它用的是 `assets/demos/serai-pop/` 里的真实图，
> 已挂上 `landing-page.html` 的作品展示区。以下文中提到暖陶土版的段落是历史记录，读的时候留意。
...
**深浅色策略跟暖陶土版不同**：Pop 版**默认永远浅色**，不跟访客的系统主题走。深色调色板还在，但要在 `<html>` 上加 `data-theme="dark"` 才会生效。理由是给客户看的 demo 不该因为对方电脑是深色主题就变一个样。
