      ...
      <details class="ap-steps">
        ...
        <ol>
          ${SHORTCUT_URL
            ...
            : `<li>在「快捷指令」app 新建一个快捷指令，取名「小帐本记帐」，依次加入：
                 <b>获取变量「快捷指令输入」的「金额」「商家」「名称」</b>，以及<b>当前日期</b>（格式设为 ISO 8601）。</li>
               <li>再加「获取 URL 内容」：网址贴上连接码，方法选 POST，请求体选 JSON，
                 字段 <code>t</code>（日期）、<code>amount</code>、<code>merchant</code>、<code>card</code>（卡名）。</li>`}
